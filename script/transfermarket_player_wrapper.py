...
for line in f:
    ...
    if soup.find("div", {"class": "dataErfolge hide-for-small"}):
        profile["club honors"] = dict()
        club_wins = soup.find("div", {"class": "dataErfolge hide-for-small"})
        for a in club_wins.find_all('a'):
            profile["club honors"][a.get("title").strip().lower()] = int(a.find("span", {"class": "dataErfolgAnzahl"}).text) if a.find("span", {"class": "dataErfolgAnzahl"}) else "none"
